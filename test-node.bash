...
num_volumes=`docker volume ls --filter label=com.docker.compose.project=arbitrum-nitro-testnode -q | wc -l`
...
if $force_init; then
    ...
    docker compose down
    leftoverContainers=`docker container ls -a --filter label=com.docker.compose.project=arbitrum-nitro-testnode -q | xargs echo`
    if [ `echo $leftoverContainers | wc -w` -gt 0 ]; then
        docker rm $leftoverContainers
    fi
    docker volume prune -f --filter label=com.docker.compose.project=arbitrum-nitro-testnode
    leftoverVolumes=`docker volume ls --filter label=com.docker.compose.project=arbitrum-nitro-testnode -q | xargs echo`
